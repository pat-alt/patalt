...
We will explain `REVISE` through a short tale of cats and dogs. The protagonist of our story is Kitty `r emo::ji("cat")`, a young cat that identifies as a dog. Unfortunately though, she is not very tall and her tail, though short for a cat, is longer than that of the average dog (Figure \@ref(fig:density)).
...
To her dismay, Kitty has been recognized as a cat by a linear classifier $g_n(X)$ that we trained through stochastic gradient descent (once again interested readers may find technical details and code in the annex below). Figure \@ref(fig:class) shows the resulting linear separation in the attribute space with the decision boundary in solid black and Kitty's location indicated through a red circle. Can we provide individual recourse for Kitty?
...
```{r class, fig.asp=1, fig.cap="Linear separation of cats and dogs in the 2-dimensional attribute space with the decision boundary of the fitted classifier in solid black. Kitty's location is indicated through the red circle."}
...
1. Initialize $\mathbf{x}_i'^{(0)}$ - the attributes that will be revised recursively. Kitty's original attributes seem like a reasonable place to start.
...
3. Return $\delta=\mathbf{x}_i'^{(0)}-\mathbf{x}_i$, that is the individual recourse for Kitty.
...
While hopefully Kitty’s journey has provided you with some useful intuition, the story is of course very silly. Even if your cat ever seems to signal that she wants to be dog, helping her cross that decision boundary will be very tricky. Some attributes are simply immutable or very difficult to change, which Joshi et al. (2019) do not fail to account for in their approach. Their proposed methodology offers a simple and ingenious approach towards providing individual recourse. Instead of bothering with Black-Box interpretability, why not simply provide a quick remedy in case things go wrong?

That idea certainly has its merit. As this post has hopefully shown, `REVISE` is easily understood and readily applicable. It could be a very useful tool to provide individual recourse in many real-world applications. As the implementation of our simplified version of `REVISE` demonstrates, researchers should find it relatively easy to develop the methodology further and tailor it to specific use cases. The simpler version here, for example, may be useful in settings where the dimensionality is relatively small and one can reasonably model the distribution of attributes without the need for generative models.

Still, you may be wondering: if the original classifier is based on poorly defined rules and proxies, then what good does `REVISE` really do? Going back to the example of high-school teacher Sarah Wysocki, one of the key attributes determining teachers' evaluations was their students' performance. Realizing this, some teachers took the shortest route to success by artificially inflating their students' test scores. That same course of action may well have been suggested by `REVISE`. As a matter of fact, Joshi et al. (2019) show that this very property of `REVISE` may be useful in detecting weaknesses of decision making systems before setting them loose (key contribution 3).

Nonetheless, the example of Sarah Wysocki demonstrates that approaches like `REVISE`, useful as they are, tend to provide solutions for very particular problems. In reality there are many such problems and research on trustworthy AI will therefore need to tackle the issue from various angles. A few places to start include the question of dealing with data that is inherently biased, improving ad-hoc and post-hoc model interpretability and continuing efforts around causality-inspired AI.
...
In my blog posts I aim to implement interesting ideas from scratch even if that usually means that things need to undergo some sort of simplification. The benefit of this approach is that experience is educationally very rewarding - both for myself and hopefully my readers. The two sections of this annex show how `REVISE` and linear classification can be implemented in R. 

### `REVISE` (simplified)

As flagged above, we use a slightly simplified version of algorithm presented in @joshi2019towards. In particular, we do not incorporate the threshold on the likelihood, which the authors implement through generative models. We will also pretend here that all attributes are mutable. 

Let $y\in\{-1,1\}$ be a binary outcome variable, $X\in\mathbb{R}^d$ a feature matrix containing individuals' attributes and $g_n(X)$ a corresponding data-dependent classifier. Suppose $y_i=-1$ - the negative outcome - for some individual characterized by attributes $\mathbf{x}_i$. Then we want to find $\mathbf{x}_i'$ closest to $\mathbf{x}_i$ such that the classifier assigns the positive outcome (primary constraint) and the likelihood of the sample $p(\mathbf{x}_i')$ exceeds some threshold $\gamma$ (secondary constraint).

In order to identify the smallest set of changes $\delta_i=\mathbf{x}_i^{'}-\mathbf{x}_i$ necessary for individual $i$ to achieve a label switch from $g(\mathbf{x}_i^{'})=1$, we use gradient descent with Hinge loss $\ell$ to minimize the following function 
...
Linear classification is implemented through stochastic gradient descent (SGD) under Hinge loss
...
where $\mathbf{w}$ is a coefficient vector, $\mathbf{x}_i$ is the attribute vector of individual $i$ and $y_i$ is individual's outcome. Since we apply SGD in order to minimize the loss function $\ell$ by varying $\mathbf{w}$, we need an expression for its gradient with respect to $\mathbf{w}$:
